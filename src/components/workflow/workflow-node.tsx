...
import { Bot, CheckCircle, AlertTriangle, Clock, HelpCircle } from 'lucide-react';

interface WorkflowNodeProps {
  title: string;
  type?: 'prompt' | 'decision' | 'agent-call' | 'wait' | 'api-call' | 'custom';
...
const statusIcons = {
  ...
  running: <Bot className="h-4 w-4 text-primary animate-pulse" />, // Use primary color
  ...
  return (
    <Card className={`min-w-[250px] max-w-xs bg-card/90 backdrop-blur-sm shadow-lg transition-all hover:shadow-xl hover:scale-[1.02] ${className}`}>
      <CardHeader className="pb-2 pt-4 px-4">
        ...
        <CardDescription className="text-xs text-muted-foreground capitalize">{type.replace('-', ' ')}</CardDescription>
      </CardHeader>
