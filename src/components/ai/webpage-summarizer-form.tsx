...
import { Loader2, Link, Search, FileText } from 'lucide-react';
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (output.error) {
        ...
        setError('No summary could be generated. The page might be empty or not summarizable.');
        addConsoleMessage('warn', `Webpage Summarizer: No summary generated for ${url}.`);
         addTimelineEvent({
          type: 'info',
  ...
  return (
    <div className="space-y-3">
      ...
      {result && result.summary && (
       ...
       {result && !result.summary && result.error && ( // Handles cases where tool returns error in output object but summary is empty
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
